...
MAX_watch_and_list_and_rc_in_smartQueue=`cat time.log | grep "watch and list and rc in smartQueue" | awk 'BEGIN{max=0} {if($13>max) max=$13} END{print max}'`
AVG_watch_and_list_and_rc_in_smartQueue=`cat time.log | grep "watch and list and rc in smartQueue"| awk 'BEGIN{sum=0; count=0} {sum+=$13;count+=1} END{print sum/count}'`
value90_watch_and_list_and_rc_in_smartQueue=`cat time.log | grep "watch and list and rc in smartQueue" | awk '{print $13}' | sort -n | head -n ${index90} | tail -n 1`
value99_watch_and_list_and_rc_in_smartQueue=`cat time.log | grep "watch and list and rc in smartQueue" | awk '{print $13}' | sort -n | head -n ${index99} | tail -n 1`
echo "watch and list and rc in smartQueue:     " $MAX_watch_and_list_and_rc_in_smartQueue\|$AVG_watch_and_list_and_rc_in_smartQueue\|$value90_watch_and_list_and_rc_in_smartQueue\|$value99_watch_and_list_and_rc_in_smartQueue
...
cat $log | grep USETIME | grep "watch and list and rc in smartQueue" | awk '{print $13}' > $dir/watch_and_list_and_rc_in_smartQueue
